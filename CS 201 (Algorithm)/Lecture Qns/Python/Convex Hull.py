...
class Solution():
    ...
    def merge_hulls(self, left_hull, right_hull):
        ...
        while True:
            if self.y_intercept(left_hull[i], right_hull[(j + 1) % len(right_hull)], mid_x) > self.y_intercept(
                    left_hull[i], right_hull[j], mid_x):
                j = (j + 1) % len(right_hull)
            elif self.y_intercept(left_hull[(i - 1) % len(left_hull)], right_hull[j], mid_x) > self.y_intercept(
                    left_hull[i], right_hull[j], mid_x):
                i = (i - 1) % len(left_hull)
            else:
                break
